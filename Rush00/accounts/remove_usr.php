<?php
	if ($_POST["submit"])
	{
		...
		foreach($admin as $arr)
			...
			{
				echo file_get_contents("http://e1z1r7p29:8080/rush00/accounts/error.php?msg=".urlencode("Cannot remove an admin<br /><br />"));
				return ;
			}
		if (($usr = unserialize(file_get_contents("../private/usr"))))
		{
			foreach ($usr as &$arr)
				if ($_POST["login"] == $arr["login"])
				{
					...
					echo file_get_contents("http://e1z1r7p29:8080/rush00/accounts/error.php?msg=".urlencode($_POST["login"]." is no longer a user<br /><br />"));
					return ;
				}
		}
...
<body>
	...
	<form method="POST">
		...
		<br />
		<a href="../accounts/settings.php" id="button"><input type="button" name="back" value="Back" id="back"/></a>
	</form>
</body>
</html>